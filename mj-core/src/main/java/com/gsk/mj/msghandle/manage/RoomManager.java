...
import com.gsk.mj.repository.RoomRepository;
...
public class RoomManager {
    ...
    public void createRoom(Avatar avatar, RoomVO roomVO){
        ...
        RoomInfo roomInfo = roomRepository.save(roomVO.toRoomInfo());
        //表中录入房间信息
        roomList.put(roomInfo.getId(),roomLogic);
        roomCount++;
        System.out.println("(创建新房间成功)当前在线房间数量："+roomCount+":"+roomList.size());
        addUuidAndRoomId(avatar.avatarVO.getAccount().getUuid(), roomInfo.getId());
    }
    /**
     * 移除房间/销毁房间
     * @param
